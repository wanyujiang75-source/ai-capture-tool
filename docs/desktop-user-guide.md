...
“抓包”页也可以直接选择 Jenkins 构建。首次点击“打开应用”或“一键开始抓包”时，如该构建
尚未安装，工具会先下载并安装，再继续后续操作。

当前原生 V1 的主选择器以 Jenkins 最新测试包为主。本地 APK、生产包和复杂 split APK 的手动
上传仍属于浏览器/高级接口能力，不应假定已出现在原生 Jenkins 列表中。
...
1. 在“抓包”页选择设备和 Jenkins 构建。
2. 点击“打开模拟器”，等待设备在线并解锁。
3. 点击“打开应用”，确认目标 App 在模拟器前台。
4. 点击“一键开始抓包”。该按钮会再次执行环境准备，首次使用的构建会自动安装。
5. 看到 Session 编号和抓包模式后，在模拟器中操作需要分析的功能。
6. 进入“接口”页查看实时请求。
7. 完成后回到“抓包”页点击“停止抓包”。
...
页面支持暂停/继续、清空当前内存视图、Tag/消息/PID 搜索、最低级别过滤和自动滚动。
