...
function formatBodyContent(detail, part) {
  const jsonValue = detail?.[`${part}_json`];
  const textValue = detail?.[`${part}_text`];
  const kind = detail?.[`${part}_body_kind`] || "";
  const body = detail?.[`${part}_body`] || {};
  if (jsonValue !== null && jsonValue !== undefined) return formatJson(jsonValue);
  if (kind === "binary") {
    const label = part === "response" ? "响应" : "请求";
    return [
      `二进制${label}体，未按文本展示。`,
      `类型: ${body.content_type || "unknown"}`,
      `大小: ${formatByteSize(body.size_bytes)}`,
      body.path ? `文件: ${body.path}` : "",
    ].filter(Boolean).join("\n");
  }
  return formatJson(textValue);
}
...
function App() {
  ...
  const mitmWebUrl = selectedDevice ? `http://127.0.0.1:${selectedDevice.web_port}/?token=android-capture` : "";

  const loadReadiness = async (appId = selectedAppId) => {
    if (!appId || !selectedDeviceId) {
      setReadiness(null);
      return null;
    }
  ...
  const loadInstalledApps = () =>
    runAction("读取已安装 App", async () => {
      if (!selectedDeviceId) {
        throw new Error("请先发现并选择 Android 设备");
      }
      const data = await api(`/api/apps/installed?device_id=${encodeURIComponent(selectedDeviceId)}`);
  ...
  const selectedMode = selectedDeviceCapture?.health === "running"
  ...
  const canOperateSelectedApp = selectedAppCanCapture && Boolean(selectedDevice) && googleReady;
  const canInstallPackage = selectedAppCanCapture && Boolean(selectedDevice) && emulatorUnlocked && googleReady;
  const installDisabledReason = !selectedAppCanCapture
    ? "请先选择 Android 应用。"
    : !selectedDevice
      ? "请先发现并选择 Android 设备。"
    : !emulatorReady
      ? "请先连接设备或启动模拟器，等待 Android 系统启动完成后再上传更新包。"
      : !emulator?.unlocked
        ? "请先解锁模拟器后再上传更新包。"
      ...
      <section className="workspace">
        <aside className="workflow-rail">
          ...
          <div className={status?.health === "running" ? "workflow-step complete" : "workflow-step"}>
            <span>03</span>
            <strong>抓包</strong>
            <small>auto / system / flutter-socks</small>
          </div>
        ...
        <div className="primary-column">
          <Panel title="当前任务" eyebrow="Capture Control" className="task-panel">
            ...
            <div className="task-grid">
              <div className="metric-card">
                <span>当前设备</span>
                <strong>{selectedDevice?.name || selectedDevice?.adb_serial || emulator?.adb_serial || "未发现设备"}</strong>
                <small>{emulatorReady ? "在线，可用于抓包" : "连接设备或启动模拟器后刷新"}</small>
              </div>
              <div className="metric-card">
                <span>目标应用</span>
              ...
              <div className="metric-card">
                ...
                <small>{selectedDevice ? selectedDeviceCapture?.proxy || `proxy ${selectedDevice.proxy_port} / web ${selectedDevice.web_port}` : "选择设备后生成端口"}</small>
              </div>
            </div>
          ...
          >
            {selectedSession ? (
              <>
                <ModeAttemptSummary session={selectedSession} flowCount={flows.length} />
                <FlowAnalysis
                  flows={visibleFlows}
          ...
          <Panel title="应用库" eyebrow="Apps" className="side-panel">
            ...
            <details className="drawer">
              ...
              <div className="form">
                ...
                <button className="secondary" onClick={loadInstalledApps} disabled={loading || !selectedDevice}>读取当前设备已安装 App</button>
                {installedApps.length ? (
                  <div className="app-button-list">
                    {installedApps.slice(0, 30).map((item) => (
                      <button
                        type="button"
                      ...
                      >
                        ...
                        <code>{item.activity || "Activity 可自动解析"}</code>
                      </button>
                    ))}
                  </div>
                ) : null}
              ...
              <select value={selectedDeviceId} onChange={(event) => onSelectDevice(event.target.value)}>
                {(setup?.devices || []).map((device) => (
                  <option key={device.device_id} value={device.device_id}>
                    {device.device_id} · {device.name || device.adb_serial}
                  </option>
                ))}
              </select>
            </div>
...
function DevicePool({ devices, selectedDeviceId, onSelect }) {
  ...
  return (
    <div className="device-pool" aria-label="已连接设备">
      {devices.map((device) => {
    ...
    <section className="network-diagnostics">
      <div className="network-diagnostics-head">
        <div>
          ...
          <small>检查本机端口是否被其他项目占用，并切换设备维护网络或抓包网络。</small>
        </div>
...
function ModeAttemptSummary({ session, flowCount }) {
  const attempts = session?.mode_attempts || [];
  const requestedMode = session?.requested_mode || session?.mode;
  if (!attempts.length && requestedMode !== "auto") return null;
  return (
    <div className="mode-summary">
      <div>
        <strong>抓包模式</strong>
        <small>{requestedMode === "auto" ? `auto -> ${session.mode}` : session.mode}</small>
      </div>
      {attempts.length ? (
        <div className="mode-attempt-list">
          {attempts.map((attempt) => (
            <span className={attempt.status === "running" || attempt.status === "passed" ? "badge" : "badge muted-badge"} key={`${attempt.mode}-${attempt.status}`}>
              {attempt.mode}: {attempt.status}
            </span>
          ))}
        </div>
      ) : null}
      {requestedMode === "auto" && flowCount === 0 ? (
        <p className="muted">自动模式已启动。若操作 App 后仍无接口，请在高级区手动切换 system 或 flutter-socks 复测。</p>
      ) : null}
    </div>
  );
}
...
function FlowDetailPanel({ detail, curl, activeTab, onTabChange }) {
  ...
  const responseBody =
    String(detail.status) === "NO_RESPONSE" && !detail.response_json && !detail.response_text
      ? "未捕获响应"
      : formatBodyContent(detail, "response");
    ...
    <div className="detail">
      ...
      {activeTab === "request" ? (
        <div className="detail-stack">
          ...
          <section>
            <h3>Request Body</h3>
            <pre>{formatBodyContent(detail, "request")}</pre>
          </section>
          <section>
            <h3>Request Files</h3>
