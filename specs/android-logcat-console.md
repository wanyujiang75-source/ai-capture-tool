...
在原生 macOS 桌面端左侧现有“日志”Tab 中提供接近 Xcode Console 的 Android 运行日志体验。用户无需打开 Android Studio 或终端；连接模拟器或已授权的 Android 真机后，页面自动发现设备并读取 Logcat，可在“应用 / 系统 / 崩溃”三种来源之间切换。

## 用户界面

- 左侧栏继续使用现有“日志”入口，不新增重复导航项。
- 页面顶部展示设备选择、前台应用识别状态、连接状态和当前日志来源，并提供“刷新设备”。单台设备也显示其类型，避免用户无法确认当前读取的是模拟器还是真机。
- 使用三段式切换：
  - `应用`：只读取当前目标应用 PID 的日志。
  - `系统`：读取当前设备主 Logcat 缓冲区。
  - `崩溃`：读取当前设备 `crash` 缓冲区。
- 工具栏提供：暂停/继续、清空当前视图、关键字搜索、最低日志级别、自动滚动。
- 每条日志展示精确到毫秒的时间、级别、Tag 和完整消息；PID/TID 不占用普通列表列。级别使用克制的颜色区分：Verbose/Debug 灰色、Info 默认色、Warn 橙色、Error/Fatal 红色。
- 默认进入“应用”来源。设备在线时优先识别当前前台应用并自动启动；应用尚未运行时显示“等待应用运行”，应用进程出现后自动接入，无需依赖旧的全局应用选择。
- 真机未授权时分别提示 USB 授权与无线调试配对动作；真机离线或断开时显示对应恢复动作，不显示 ADB 原始错误。
...
- 每次启动或重新连接 `adb logcat` 时使用 `-T 1`，只接入最新一条附近及之后的新输出，避免把整个 Android 环形缓冲区回放到桌面端。
- 每台设备使用单调递增 cursor；缓冲区最多保留 5,000 条或 2 MiB，先到上限者生效。
- 客户端超过 30 秒没有轮询时自动停止对应 `adb logcat` 子进程。
- 后端退出、设备释放、系统休眠时停止相关 Logcat 会话。
- 桌面端离开日志页时主动调用停止接口；30 秒回收仅作为异常退出兜底。
- V1 不把日志写入 SQLite、抓包目录或普通文件。

## 设备发现与真机准入

- `adb devices -l` 的 `device`、`unauthorized`、`offline` 状态都必须被解析，未授权设备用于生成引导但不得作为可读日志目标。
- 已授权真机使用稳定且不与模拟器池冲突的设备 ID；重复刷新复用同一记录和端口。
- 日志设备使用独立发现接口；它可复用已有模拟器记录，也可建立禁用状态的日志专用真机记录，但不得覆盖、改名或停用已有 AVD/模拟器池记录。
- 真机连接类型分为 `usb` 与 `wireless`；普通界面展示易懂名称，序列号仅在设备详情或运行检查中出现。
- 真机 Logcat 只要求 ADB 调试授权，不要求 Root、Frida、证书、代理或 Google 登录。

## API

### 发现日志设备

`GET /api/log-devices/discover`

返回已授权在线的模拟器和真机，以及仅用于页面授权引导的 `blocked_devices`。原 `/api/devices/discover` 保持抓包设备池语义，日志页不得调用它。
...
- `LogsView` 进入时触发设备发现并刷新列表；应用来源通过前台应用接口解析包名，再把设备和包名变化传给 `LogcatController`。
...
- 离开“日志”页时停止轮询并主动停止对应后端会话；后端 30 秒 TTL 只负责崩溃或网络中断兜底释放。
- ADB 断连或授权失效后停止高频轮询，等待用户刷新设备或连接恢复。
- 搜索和级别过滤在 Swift 客户端对当前有界缓冲执行，不额外请求后端。
- 桌面端按连续条目的时间、PID、TID、级别和 Tag 合并同一次多行输出；内容完全相同的连续逻辑日志折叠并显示总次数。搜索、复制和计数均使用折叠后的逻辑日志。
- 清空时推进客户端读取代次并取消旧轮询；清空前已经发出但稍后返回的分页响应不得重新写入界面。随后从后端返回的新 cursor 继续实时读取。
- 表格模式对超过 3 行或 360 个字符的消息显示紧凑预览，并提供“展开完整内容/收起内容”；展开只改变显示，不改变日志数据。
- 纯文本模式、搜索与“复制全部”始终使用完整逻辑日志，不使用表格预览文本。
...
- 离开页面后立即停止，释放设备或退出后端后也无该设备残留 `adb logcat` 进程。
- 已授权 USB/无线真机可读取三类日志，未授权真机有明确授权引导，且整个流程不要求 Root。
- 连续刷新设备不会改变真机 ID、重复占用端口或破坏已有模拟器配置。
- 超长单行内容与异常堆栈不会默认撑高整张表格，展开后仍可选择完整文本。
- Python、Swift 和最终 `.app` 构建测试通过。

## 非目标

- 不实现 Windows/Linux/iOS 日志。
- 不为真机自动开启开发者选项、代替用户确认 USB 调试授权或绕过厂商安全限制。
