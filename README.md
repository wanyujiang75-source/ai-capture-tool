...
1. 打开桌面 App。页面默认进入“抓包”，内部服务会自动启动，不需要手动打开网页或后端。
2. 如果尚未安装应用，先进入“安装应用”，从 Jenkins 选择测试包或点击“选择 APK”。模拟器未启动时，先回到“抓包”点击“启动模拟器”。
3. 在“抓包”页点击一次“开始抓包”。工具会自动启动模拟器、等待 Android 启动、检查网络和抓包组件，并在每一步显示当前状态。
4. 页面显示“等待解锁”时，在模拟器中解锁屏幕；显示“等待应用”时，打开需要分析的应用。工具识别后会自动继续，不需要再次点击准备按钮。
5. 如果应用需要 Google 账号，先在模拟器中打开 Play Store 并手动登录；账号密码不会由桌面端保存。
6. 页面显示“正在抓取‘应用名’”后，操作需要分析的功能。在“接口”页可实时查看 Request、Response 和 cURL。
7. 必要时进入“日志”，工具会自动发现模拟器及已授权的 USB/无线 Android 真机；选择设备后可按关键词、标签和级别查看当前应用的运行日志、系统日志或崩溃信息。
8. 完成后回到“抓包”点击“停止抓包”。结果会保留；之后可立即开始下一次抓包。

主导航固定为“抓包 / 安装应用 / 接口 / 日志”。环境和底层组件只在工具栏“运行检查”中展示，普通操作页不会直接显示后端原始错误、端口或组件命令。

真机日志只要求 Android 已开启开发者选项和 USB/无线调试，并授权当前 Mac，不要求 Root、Frida、Google 登录或代理配置。该能力用于查看 Logcat，不代表当前版本已支持真机网络抓包；完整连接步骤见[桌面端使用指南](docs/desktop-user-guide.md#7-查看日志)。
...
新版启动时会校验内嵌后端的构建标识：空闲旧版后端仅在 PID 记录和可执行文件均确认属于抓包工具时才会被安全替换；旧版仍在抓包时会提示先停止任务；`7001` 被其他程序占用时只提示冲突，不会结束该程序。
...
1. 点击 `开始抓包`，让系统检查依赖、安装或定位 Google Play 镜像、创建并启动默认模拟器。
...
启动抓包前，后端会检查设备在线、App 可启动、端口可用，以及 `flutter-socks` 模式下 Frida 是否可连接。端口被其他项目占用时会提示具体端口，不会结束未知进程。自动校验过程中如果某个模式失败，会停止当前临时抓包并清理 Android 代理后再尝试下一个模式。
