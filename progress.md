...
| L10 | DOING | 在桌面发行包中提供安装说明和前置准备条件 | `goal.md`, `specs/release-install-guide.md`, `progress.md`, `release/安装说明.md`, `release/package.sh`, `tests/test_native_app_packaging.py` | ZIP 根目录包含 App 与安装说明；说明覆盖前置条件、安装、首次使用和常见问题；开发与正式包共用；打包、SHA、签名、架构、冷启动及远端资产校验通过 | 已确认当前 `20260916-baccb23` ZIP 仅包含 App，用户安装说明只存在于仓库与 Release 页面 | 先以缺失说明的回归测试复现，再实现打包并更新 GitHub Release |
