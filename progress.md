...
| UX7 | DOING | 修复升级后误连接旧版内嵌后端并重发桌面包 | `capture_console/app.py`, `tests/test_console_api.py`, `macos-native/Sources/AICaptureNativeApp/RuntimeManager.swift`, `macos-native/Tests/AICaptureNativeAppTests/RuntimeManagerTests.swift`, `macos-native/scripts/build-app.sh`, `tests/test_native_app_packaging.py`, `README.md`, `docs/desktop-user-guide.md`, `progress.md` | `/api/status` 返回构建标识；同版本后端可复用；不匹配版本只接管 PID 文件记录且可验证为本应用运行时的进程；未知端口占用不被结束；全量测试、解压启动、签名、SHA-256 和 GitHub Release 验证通过 | 已真实复现旧窗口退出后 PID `18716` 继续监听 `7001`，新 App 因仅检查 HTTP 200 而复用旧后端；当前发布上传已暂停 | 先以失败测试固定版本握手与进程归属边界 |
