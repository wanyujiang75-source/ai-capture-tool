...
| L11 | DOING | 修复停止抓包后界面仍保持运行状态 | `goal.md`, `specs/capture-stop-semantics.md`, `progress.md`, `capture_console/app.py`, `tests/test_console_api.py`, `macos-native/Sources/AICaptureNativeApp/AppCopy.swift`, `macos-native/Sources/AICaptureNativeApp/Models.swift`, `macos-native/Sources/AICaptureNativeApp/AppState.swift`, `macos-native/Tests/AICaptureNativeAppTests/CaptureWorkflowTests.swift` | 停止成功与代理恢复分别报告；部分清理失败不阻塞停止；真正停止失败保留活动任务；Python、Swift、App 构建、签名及离线设备真实接口验收通过 | 已复现：Session #43 后端为 `stopped`，但旧接口因 `adb: device 'emulator-5554' not found` 返回 `ok=false`；桌面端因此保留运行状态 | 已补后端与桌面端失败回归，先确认 RED，再修改停止契约 |
