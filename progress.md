...
| L4 | DOING | 补齐桌面端使用文档并发布功能分支 | `.gitignore`, `README.md`, `macos-native/README.md`, `docs/desktop-user-guide.md`, `specs/functional-acceptance.md`, `task_plan.md`, `progress.md` 及已完成实现变更 | 普通用户、开发和排错说明与当前实现一致；提交不含凭据、运行数据、APK、抓包结果或临时产物；全量验证通过后提交并推送 | 已完成工作树、远端和敏感信息初检，新增分层使用指南并同步 Google 登录策略与验收状态 | 执行 Markdown/敏感信息检查、全量测试、构建和暂存清单复核 |
