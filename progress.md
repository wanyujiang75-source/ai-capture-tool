...
| F1 | TODO | 实现前台 App 解析与动态登记 API | `capture_console/foreground.py`, `capture_console/runner.py`, `capture_console/app.py`, `tests/test_foreground_target.py`, `tests/test_console_api.py`, `progress.md` | 解析三类 resumed component；系统界面不登记；未知已安装 App 幂等创建；已有 App 元数据不被覆盖；设备异常返回稳定状态；目标测试通过 | 尚未执行 | 先写失败测试并确认 RED |
